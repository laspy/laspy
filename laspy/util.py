...
fmtLen = {"<l": 4, "<L": 4, "<h": 2, "<H": 2, "<B": 1,
          "<b": 1, "<f": 4, "<s": 1, "<d": 8, "<Q": 8, "<q": 8}
...
npFmt = {"<l": "i4", "<L": "u4", "<h": "i2", "<H": "u2", "<B": "u1",
         "<f": "f4", "<s": "S1", "<d": "f8", "<q": "i8", "<Q": "u8", "<b": "i1"}
...
class Format():
    ...
    def build_header(self, fmt):
        ...
        self.add("file_sig", "ctypes.c_char", 4,
                 pack=True, overwritable=self.overwritable)
        ...
        self.add("version_major", "ctypes.c_ubyte",
    ...
    def __init__(self, reader, bytestr=False, unpacked_list=False, nice=False):
        ...
        if bytestr != False:
            ...
            raise LaspyException(
                "No byte string or attribute list supplied for point.")
        if nice:
            self.make_nice()
