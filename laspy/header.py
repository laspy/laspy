...
class ParseableVLR():
    def parse_data(self):
        ...
        if "LASF_Projection" in self.user_id and self.record_id == 2111:
            ...
            self.body_fmt.add("ogc_math_transform",
                              "ctypes.c_char", self.rec_len_after_header)

        elif "LASF_Projection" in self.user_id and self.record_id == 2112:
            # OGC Coordinate System WKT
            self.body_fmt = util.Format(None)
            self.body_fmt.add("coordinate_system_wkt",
                              "ctypes.c_char", self.rec_len_after_header)
            ...
            for sKeyEntry in xrange(int(bytes_left / 8)):
                ...
                self.body_fmt.add("wTIFFTagLocation_%i" %
                                  sKeyEntry, "ctypes.c_ushort", 1)
                self.body_fmt.add("wCount_%i" %
                                  sKeyEntry, "ctypes.c_ushort", 1)
                self.body_fmt.add("wValue_Offset_%i" %
                                  sKeyEntry, "ctypes.c_ushort", 1)
            ...
            self.body_fmt = util.Format(None)
            self.body_fmt.add("GeoASCIIParamsTag",
                              "ctypes.c_char", self.rec_len_after_header)
            ...
            if self.rec_len_after_header % 257 != 0:
                print(
                    "WARNING: Invalid body length for header flight line lookup, not parsing.")
                return
            ...
            self.body_fmt = util.Format(None)
            self.body_fmt.add("text_area_description",
                              "ctypes.c_char", self.rec_len_after_header)
...
class EVLR(ParseableVLR):
    ...
    def build_from_reader(self, reader):
        ...
        self.rec_len_after_header = reader.read_words(
            "rec_len_after_header", "evlr")
...
class HeaderManager(object):
    ...
    def flush(self):
        ...
        for dim in self._header.format.specs:
            self.reader.set_header_property(
                dim.name, self._header.__dict__[dim.name])

    def pull(self):
        '''Load all header data from file into the header.Header instance.'''
        for dim in self._header.format.specs:
            self._header.__dict__[
                dim.name] = self.reader.get_header_property(dim.name)
    ...
    def set_waveform_data_packets_internal(self, value):
        self.assertWriteMode()
        raw_encoding = self.reader.binary_str(self.get_global_encoding(), 16)
        self.set_global_encoding(self.reader.packed_str(
    ...
    def get_wkt(self):
        if self.data_format_id > 5:
            ...
            raise util.LaspyException(
                "WKT not present in data_format_id " + str(self.data_format_id))

    def set_wkt(self, value):
        self.assertWriteMode()
        if self.data_format_id > 5:
            raw_encoding = self.reader.binary_str(
                self.get_global_encoding(), 16)
            self.set_global_encoding(self.reader.packed_str(
                raw_encoding[0:4] + str(value) + raw_encoding[5:]))
        else:
            raise util.LaspyException(
                "WKT not present in data_format_id " + str(self.data_format_id))
    ...
    def set_schema(self, value):
        if self.file_mode != "w":
            raise NotImplementedError(
                "Converseion between formats is not supported.")
        else:
    ...
    start_wavefm_data_rec = property(
        get_start_wavefm_data_record, set_start_wavefm_data_record, None, None)

    def get_start_first_evlr(self):
        if not self.version == "1.4":
            raise util.LaspyException(
                "EVLRs are present explicitly only in version 1.4")
    ...
    start_first_evlr = property(
        get_start_first_evlr, set_start_first_evlr, None, None)
    ...
    def get_legacy_point_records_count(self):
        if not self.version == "1.4":
            raise util.LaspyException(
                "Point records count is only denoted as legacy in version 1.4 files.")
    ...
    def get_legacy_point_return_count(self):
        if not self.version == "1.4":
            raise util.LaspyException(
                "Point return count is only denoted as legacy in version 1.4 files.")
