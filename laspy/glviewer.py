...
def run_glviewer(file_object, mode, dim):
    pcl_image(file_object, mode, dim)
    return(0)
...
class VBO_Provider():
    def __init__(self, file_object, vbsize, means, mode, dim, scaled=True):
        ...
        while(start_idx < len((file_object))):
            ...
            try:
                ...
                print("Buffering points " + str(start_idx) +
                      " to " + str((end_idx)))
    ...
    def slice_file(self, start_idx, end_idx, means, scaled):
        if scaled:
            ...
            scale = np.array(self.file_object.header.scale +
                             [0, 0, 0], dtype=np.float64)
            dat = (np.array(np.vstack((self.file_object.X[start_idx:end_idx],
                                       self.file_object.Y[start_idx:end_idx],
                                       self.file_object.Z[start_idx:end_idx],
                                       np.zeros(end_idx - start_idx),
                                       np.zeros(end_idx - start_idx),
                                       np.zeros(end_idx - start_idx))).T) - means)
            dat *= (scale * 100)
            return(dat)
    ...
    def set_color_mode(self, mode, dim, start_idx, end_idx, data):
        if (mode == "default"):
            if (all([x in self.file_object.point_format.lookup for x in (
                    "red", "green", "blue")])):
                ...
                if ((all(self.file_object.red[0:len(self.file_object):idx_step] == 0)) and
                    ...
                    print(
                        "Warning: Color data appears empty, using intensity mode. Specify -mode=rgb to override")
                    mode = "intensity"
                else:
                    mode = "rgb"
            else:
                mode = "intensity"
        if mode == "rgb" and "red" not in self.file_object.point_format.lookup:
            print("Color data not found in file, using intensity")
            mode = "intensity"
        if mode in ["grey", "greyscale", "intensity"]:
            if isinstance(self.allcolor, bool):
                self.allcolor = self.file_object.reader.get_dimension(
                    dim) / float(np.max(self.file_object.reader.get_dimension(dim)))
            scaled = self.allcolor[start_idx:end_idx] + 0.1
            col = np.array(
                (np.vstack((scaled, scaled, scaled)).T), dtype=np.float32)
            data[:, 3:6] += col
            return(data)
            ...
            if isinstance(self.allcolor, bool):
                self.allcolor = self.heatmap(
                    self.file_object.reader.get_dimension(dim))
            col = self.allcolor[start_idx:end_idx]
            data[:, 3:6] += col
            return(data)
        elif mode == "rgb":
            _max = max(np.max(self.file_object.red), np.max(
                self.file_object.green), np.max(self.file_object.blue))
            _min = min(np.min(self.file_object.red), np.min(
                self.file_object.green), np.min(self.file_object.blue))
            diff = _max - _min
            col = np.array(np.vstack((self.file_object.red[start_idx:end_idx],
                                      self.file_object.green[start_idx:end_idx],
                                      self.file_object.blue[start_idx:end_idx])).T,
                           dtype=np.float32)
            col -= _min
            col /= diff
            data[:, 3:6] += col
    ...
    def heatmap(self, vec, mode=1):
        ...
        if mode == 1:
            col = np.array(np.vstack(
                (red**4, np.sqrt(0.0625 - (0.5 - red)**4), (1 - red)**4)), dtype=np.float32).T
        else:
            col = np.array(
                np.vstack(
                    (red**4,
...
class pcl_image():
    ...
    def main(self):
        ...
        glut.glutInitDisplayMode(
            glut.GLUT_RGB | glut.GLUT_DOUBLE | glut.GLUT_DEPTH)
    ...
    def read_data(self, mode, dim):
        ...
        self.data_buffer = VBO_Provider(
            self.file_object, 1000000, means, mode, dim, scaled)
