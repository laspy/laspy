    ...
    def __init__(self, filename, manager):
        ...
        if self.mode in ("w"):
            ...
            try:
                ...
                raise laspy.util.LaspyException("Error determining compression: " + str(e))
...
class FileManager(object):
    ...
    def setup_read_write(self, vlrs, evlrs, read_only=True):
        ...
        if self.header.version in ("1.3", "1.4"):
            ...
            self.evlrs = []
        if vlrs is not False:
            self.set_vlrs(vlrs)
        if evlrs is not False:
            self.set_evlrs(vlrs)
    ...
    def get_next_point(self):
        ...
        if self._current is None:
            raise laspy.util.LaspyException("No Current Point Specified," + " use Reader.GetPoint(0) first")
    ...
    def get_dimension(self, name):
        ...
        try:
            ...
            raise laspy.util.LaspyException("Dimension: " + str(name) + " not found. Existing dimensions: " + ", ".join(self.point_format.lookup.keys()))
    ...
    def _get_raw_datum(self, rec_offs, spec):
        ...
        return(self.data_provider._mmap[(rec_offs + spec.offs):(rec_offs + spec.offs + spec.num * spec.length)])
    ...
    def _get_datum(self, rec_offs, spec):
        ...
        unpacked = map(lambda x: struct.unpack(spec.fmt, data[x * spec.length:(x + 1) * spec.length])[0], xrange(spec.num))
        if spec.pack:
...
class Writer(FileManager):
    ...
    def set_evlrs(self, value):
        ...
        if not all([x.isEVLR for x in value]):
            raise laspy.util.LaspyException("set_evlrs requers an iterable object " + "composed of :obj:`laspy.header.EVLR` objects.")
        elif self.mode == "w+":
            ...
            raise(laspy.util.LaspyException("set_evlrs requires the file to be opened in a " + "write mode, and must be performed before point information is provided." + "Try closing the file and opening it in rw mode. "))
    ...
    def set_vlrs(self, value):
        ...
        if not all([x.isVLR for x in value]):
            raise laspy.util.LaspyException("set_vlrs requers an iterable object " + "composed of :obj:`laspy.header.VLR` objects.")
        elif self.mode == "w+":
            raise NotImplementedError
        elif self.mode == "rw":
    ...
    def define_new_dimension(self, name, data_type, description=""):
        ...
        new_dimension = laspy.header.ExtraBytesStruct(name=name,
                                                      data_type=data_type,
        ...
        if len(eb_vlrs) + len(eb_evlrs) > 1:
            ...
            if len(eb_vlrs) == 1:
                ...
                new_pt_fmt = laspy.util.Format(self.point_format.fmt, extradims=extra_dimensions)
                self.point_format = new_pt_fmt
                self.set_header_property("data_record_length", self.point_format.rec_len)
    ...
    def _set_raw_points(self, new_raw_points):
        ...
        def f(x):
            self.data_provider._mmap[self.point_refs[x]:self.point_refs[x] + self.header.data_record_length] = new_raw_points[x]
        map(f, idx)
        self.data_provider.point_map()
    ...
    def _set_raw_datum(self, rec_offs, spec, val):
        ...
        self.data_provider._mmap[rec_offs + spec.offs:rec_offs + spec.offs + spec.num * spec.length] = val
        return
    ...
    def _set_datum(self, rec_offs, dim, val):
        ...
        if dim.num != dimlen:
            raise(laspy.util.LaspyException("Fields must be replaced with data of the same length. " + str(dim.name) + " should be length " + str(dim.num) + ", received " + str(dimlen)))
        ...
        def f(x):
            ...
            self.data_provider._mmap[(x * dim.length + rec_offs + dim.offs):((x + 1) * dim.length + rec_offs + dim.offs)] = outbyte
    ...
    def set_withheld(self, withheld):
        ...
        if self.header.data_format_id in (6, 7, 8, 9, 10):
            ...
            class_byte = self.get_raw_classification()
            self.raise_if_overflow(withheld, 1)
            out_byte = self.bitpack((class_byte, withheld), ((0, 7), (0, 1)))
            self.set_dimension("raw_classification", out_byte)
        return
