...
class lasverify(object):
    ...
    def parse_args(self):
        ...
        parser.add_argument('-b', type=bool,
                            help='''Attempt to compare incompatible sub-byte sized data fields if present? (slow)''',
                            default=False)
    ...
    def verify(self):
        ...
        try:
            ...
            print(f"Error reading in files: {error}")
            quit()
        ...
        failed = 0
        for dim in dims:
            if not SUB_BYTE_COMPATIBLE and dim in ("raw_classification",
